...
void sort_input(std::vector<std::vector<int>>& input, int op) { 
	std::vector<SortingOption> options;
	init_options(options);
	SortingOption option = options[op];
...
int my_split(std::vector<int>& tokens, std::string& line) {
...
bool get_num(std::string& str, int& cell) {
	int idx = 0;
	bool minus_flag = 0;
	if (str[idx] == '-') {
		idx++;
